...
static struct ili9341_lcd_s *esp32s3_initializa_ili9342c(int spi_port)
{
   ...
   * Note: GPIO 48 (DISPLAY_RST) is shared with the GT911 touch controller
   * and is managed by the touchscreen driver. Do NOT toggle it here —
   * the ILI9342C config table already includes a SOFTWARE_RESET command
   * that achieves the same result without disturbing the touch controller.
   */
